...
stitchedpython_df.replace([np.inf, -np.inf], np.nan, inplace=True)
stitchedpython_df.dropna(subset=['OD'], inplace=True)
        ...
        for sec_dilution in sec_dilutions:
            ...
            sub_df_expand = pd.concat(
                [sub_df.loc[[0], ['antigen',
                             'serum type', 'serum cat', 'serum dilution','serum ID',
                             'secondary ID',
                             'secondary dilution',
...
#
#     plt.savefig(os.path.join(fig_path, '{}_{}_{}_fit.jpg'.format('Neg pool', 'Pos pool', 'fits full range')),
#                              dpi=300, bbox_inches='tight')


# %% Plots that show CR3022 fit for 3 dilutions with positive and negative pools as scatter points

fig_path = os.path.join(data_folder1, 'pysero_plots temp')
os.makedirs(fig_path, exist_ok=True)
pipeline = 'python'
# sera_list1= ['Neg pool', 'Pos Pool','CR3022']
# sera_list2= ['Neg pool', 'Pos Pool']
# sera_list= ['CR3022']
sera_list1 =['Neg pool 1/50 + CR3022' ]
sera_list2 = ['Neg pool 1/200 + CR3022']
sera_list3 = ['Neg pool 1/800 + CR3022']
# sera_list4 = ['Neg pool 1/50 + CR3022','Neg pool 1/200 + CR3022','Neg pool 1/800 + CR3022']
sera_fit_list = [' '.join([x, 'fit']) for x in sera_list1]
markers = 'o'
hue = 'serum ID1'
hue1 = 'serum ID1'
sec_dilutions = [2e-4]
...
#antigens = ['SARS CoV2 spike 125', 'SARS CoV2 RBD 250']
serum_df = stitchedpython_df[(stitchedpython_df['pipeline']==pipeline) & stitchedpython_df['serum ID1'].isin(sera_list1)
                      & stitchedpython_df['secondary dilution'].isin(sec_dilutions)]
#serum_df = stitchedpython_df[(stitchedpython_df['pipeline']==pipeline) & stitchedpython_df['serum cat'].isin(sera_list1)
 #                    & stitchedpython_df['secondary dilution'].isin(sec_dilutions)]
assert not serum_df.empty, 'Plotting dataframe is empty. Please check the plotting keys'
for serdil in serum_df['serum dilution'].unique():
    sub_df = serum_df[(serum_df['serum dilution'] == serdil)]
    palette = sns.color_palette(n_colors=len(sub_df[hue].unique()))
    print('plotting...')
    g = sns.lmplot(x="serum dilution2", y="OD", col_order=antigens,
                    hue=hue, col="antigen", ci='sd', palette=palette,
                     data=sub_df, col_wrap=5, fit_reg=False, x_estimator=np.mean)
    sera_fit_list = [' '.join([x, 'fit']) for x in sera_list1]
    sub_python_df_fit=python_df_fit[(python_df_fit['pipeline']==pipeline) &
                                   python_df_fit['serum ID1'].isin(sera_fit_list) & (python_df_fit['serum dilution'] == serdil) & python_df_fit['secondary dilution'].isin(sec_dilutions)
                                    ]
    palette = sns.color_palette(n_colors=len(sub_python_df_fit[hue].unique()))
    for antigen, ax in zip(antigens, g.axes.flat):
        df_fit1 = sub_python_df_fit[(sub_python_df_fit['antigen'] == antigen)]
        sns.lineplot(x="serum dilution2", y="OD", hue=hue, data=df_fit1, style=style, palette=palette,ax=ax,
                      legend=False)

        #sns.lineplot(x="serum dilution2", y="OD", hue=hue1, data=df_fit1, palette=palette, style=style, ax=ax, legend=False)
        ax.set(xscale="log")
            # ax.set(ylim=[-0.05, 1.5])
#

    plt.savefig(os.path.join(fig_path, '{}_{}_{}_fit.jpg'.format('Cr3022 fits', 'per Ag', 'dil all')),
                             dpi=300, bbox_inches='tight')
...
# palette1=sns.color_palette("Greys", 8)
